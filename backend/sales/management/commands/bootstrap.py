...
from sales.models import Ticket, SalesProject, Company , CustomerInformation, Quotation, VendorInformation, UserProfile, SalesDepartment , BudgetBlock, ActualStateType
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        actual_state = ActualStateType.objects.create(state_type='Completed', state=closed_state, workflow_type='Project')
        ...
        root = User.objects.filter(username="admin").first() or User.objects.create_superuser("admin", "", 'doremi123')
        ...
        company = Company(schema_name='public', name='public', paid_until='2030-01-01', on_trial=False)

        company.save()

        self.stdout.write(self.style.SUCCESS(f'Successfully bootstrapped the db.\nUsername: admin\nPassword: doremi123'))
