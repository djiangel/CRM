...
import river_admin_remake
from river_admin_remake.views import get, post, delete, StateDto
from river_admin_remake.views.serializers import WorkflowStateFieldDto, CreateWorkflowDto, WorkflowDto, \
    TransitionMetaDto, TransitionDto, WorkflowMetadataDto , WorkflowClassWithPermissionsDto


@get(r'^workflow/get/(?P<pk>\w+)/$')
def get_it(request, pk):
    workflow = get_object_or_404(Workflow.objects.all(), pk=pk)
    return Response(WorkflowClassWithPermissionsDto(workflow , context={'request': request}).data, status=HTTP_200_OK)
