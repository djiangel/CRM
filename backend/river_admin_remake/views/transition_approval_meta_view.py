...
from river_admin_remake.views.serializers import RePrioritizeTransitionApprovalMetaDto, TransitionApprovalMetaDto, CreateTransitionApprovalMetaDto, ApprovalHookDto
...
def create_it(request):
    ...
    if create_transition_approval_meta_request.is_valid():
        ...
        return Response({"data": TransitionApprovalMetaDto(transition_approval_meta).data}, status=HTTP_200_OK)
    else:
