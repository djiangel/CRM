...
from river_admin_remake.views import get, post ,  delete
from river_admin_remake.views.serializers import TransitionMetaDto, CreateTransitionMetaDto, TransitionHookDto, TransitionApprovalMetaDto
...
def create_it(request):
    ...
    if create_transition_meta_request.is_valid():
        ...
        return Response(TransitionMetaDto(transition_meta,many=False).data, status=HTTP_200_OK)
    else:
...
@delete(r'^transition-meta/delete/(?P<pk>\w+)/$')
def delete_it(request, pk):
    transition_meta = get_object_or_404(TransitionMeta.objects.all(), pk=pk)
    transition_meta.delete()
